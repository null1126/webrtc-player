...
features:
  ...
  - icon: 🎯
    title: 事件驱动
    details: 完整的事件系统，覆盖连接状态（connecting/connected/disconnected/failed）、媒体权限（camera/microphone granted/denied）、ICE 候选交换等全生命周期事件。
  - icon: 📹
    title: 多源采集
    details: 支持摄像头（getUserMedia）、麦克风、屏幕录制（getDisplayMedia）等媒体源。可在推流过程中动态切换输入设备，无需重建连接。
  - icon: ⚡
    title: 高性能 & 零依赖
    details: 纯 TypeScript 实现，无任何外部运行时依赖。体积小，加载快，适合在生产环境中使用。
  - icon: 🌐
    title: 跨浏览器 & 框架无关
    details: 基于标准 WebRTC API 构建，支持 Chrome、Firefox、Safari、Edge 所有现代浏览器。可直接用于 React、Vue、Angular 或原生 JavaScript 项目。
  - icon: 🔄
    title: 流切换
    details: 支持动态切换视频源（切换摄像头、麦克风设备或屏幕录制流），无需销毁重建播放器实例，适合实时协作和监控类应用。
  - icon: 📦
    title: npm 开源
